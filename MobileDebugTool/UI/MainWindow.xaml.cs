...
public partial class MainWindow : Window
{
    ...
    private readonly LinkedList<string> _recentDeviceHistory = new();
    ...
    private async void OnRefreshDevicesClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (devices.Count > 0)
            {
                RegisterRecentDevice(devices[0]);
    ...
    private void RegisterRecentDevice(AndroidDevice device)
    {
        var key = $"{device.Serial} ({device.State})";

        var existing = _recentDeviceHistory.Find(key);
        if (existing is not null)
        {
            _recentDeviceHistory.Remove(existing);
        }

        _recentDeviceHistory.AddFirst(key);

        while (_recentDeviceHistory.Count > 5)
        {
            _recentDeviceHistory.RemoveLast();
        }

        RecentHistoryListBox.ItemsSource = _recentDeviceHistory.ToList();
    }
