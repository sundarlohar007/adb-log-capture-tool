...
public sealed class AndroidAdbService : IAndroidService
{
    ...
    public async Task<bool> InstallApkAsync(string serial, string apkPath, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(serial) || string.IsNullOrWhiteSpace(apkPath) || !File.Exists(apkPath))
        {
            return false;
        }
        ...
        var result = await ProcessRunner.RunAsync(adbPath, $"-s {serial} install -r \"{apkPath}\"", cancellationToken).ConfigureAwait(false);
